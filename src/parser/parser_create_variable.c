...
static bool set_and_return_error(
		...
		struct err_msg_t* err)
{
	...
	if (!success)
		return set_and_return_error(err, token_buffer, 0);	

	// second token is name
	success = variable_set_name(&new_variable, token_buffer->token[1].text, err);
	if (!success)
		return set_and_return_error(err, token_buffer, 1);
...
bool parser_create_local_variable(
		...
		struct err_msg_t* err)
{
	// TODO: implement this after functions are working
	return false;
}
